...
interface AddEditBrandFormProps {
  item?: ItemBrand | null;
  categoryId: number;
  reLoading: Dispatch<SetStateAction<boolean>>;
}

const AddEditBrandForm: React.FC<AddEditBrandFormProps> = ({
  item,
  categoryId,
  reLoading,
}) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const resp = item
        ? await axios.put(`/api/brand-items/${item.id}`, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          })
        : await axios.post("/api/brand-items", formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          });
